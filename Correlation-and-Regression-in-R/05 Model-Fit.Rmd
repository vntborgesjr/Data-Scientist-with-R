...
This results in an SSE value of 1913. Compare this number to the SSE for our possum model

**7. SSE, our model**

that uses tail length as an explanatory variable. The SSE in this case is 1301. The ratio of the SSE for our model to the SSE for the null model is a quantification of the variability explained by our model.
...
the point corresponding to the slugger Jose Bautista in the upper left

**4. Unusual points**

and the point belonging to speedster Juan Pierre in the lower right. Now that we have a regression model, we want to think about how individual observations might affect the slope of the line.
...
The concepts of leverage and influence will help us quantify this intuition. Leverage has a precise mathematical definition that you can see here. The specifics of the formula are not so important, but you should recognize that the leverage score h sub i for an observation is entirely a function of the distance between the value of the explanatory variable and mean of the explanatory variable. This means that points that are close to the horizontal center of the scatter plot have low leverage, while points that are far from the horizontal center of the scatter plot have high leverage. The y coordinate doesn't matter at all.
